...
class Solution(object):
    def build(self, postorder, s1, e1, inorder, s2, e2):
        ...
        idx = inorder.index(preorder[e1])
        root.left = self.build(preorder, s1, s1+(idx-s2)-1, inorder, s2, idx-1)
        root.right = self.build(preorder, s1+idx-s2, e1-1, inorder, idx+1, e2)
        return root
